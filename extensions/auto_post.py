...
class AutoPost(commands.Cog):
    ...
    @app_commands.command(name='createautopost', description='Create an automatic trade advertisement!')
    ...
    async def slash_createautopost(self, interaction: discord.Interaction, have: str, want: str, tradelink: str, channel: discord.TextChannel):
        ...
        if not interaction.user.get_role(CONTRABAND_ROLE_ID):
            infoEmbed = discord.Embed(title="You don't have access to this command.", description="This is a feature that lets traders auto their trade advertisements.\nTo get access to this yourself, check out #patreon.", color=0x313338)
            await interaction.followup.send(embed=infoEmbed, ephemeral=True)
            return
        
        
        if  not channel_id in (LOW_TIER_ID, MID_TIER_ID, HIGH_TIER_ID):
            await interaction.followup.send("Invalid channel selection. Please select a valid channel.", ephemeral=True)
            return
        ...
        async with aiosqlite.connect(DB) as conn:
            ...
            await cursor.execute('''INSERT INTO TradeAds (have, want, tradelink, channel_id, user_id) VALUES (?, ?, ?, ?, ?)
        ...
        await interaction.followup.send("Trade information stored and scheduled for autoposting!", ephemeral=True)
    ...
    async def posttask(self):
        ...
        async with aiosqlite.connect(DB) as conn:
            cursor = await conn.cursor()
            await cursor.execute('SELECT have, want, tradelink, channel_id, user_id, next_post FROM TradeAds ORDER BY datetime(next_post) ASC LIMIT 1')
        ...
        bot_webhook = discord.utils.get(webhooks, user__id=self.bot.user.id) or await ad_channel.create_webhook(name=author.name, avatar = avatar_bytes)
        ...
        infoEmbed = discord.Embed(title="What is this?", description="This is a feature that lets traders autopost their trade advertisements\nand not have to wait the entire 6-hour cooldown\nTo get access to this yourself, check out #patreon.", color=0x313338)


        await bot_webhook.send(content=f"{line}\n{trade_message}", embed=infoEmbed, username=author.name, avatar_url=avatar_url)
