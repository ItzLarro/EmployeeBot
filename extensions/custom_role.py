...
class CustomRoleButtons(discord.ui.View):
    ...
    async def accept_button(self, interaction:discord.Interaction, button:discord.ui.Button):
        most_active_member_role = discord.utils.get(self.guild.roles, name="Most Active Member")
        if most_active_member_role:
            hoist_position = most_active_member_role.position + 1
            await self.role.edit(position=hoist_position)
        else:
            await self.admin_channel.respond("Failed to find the 'Most Active Member' role.")
        await interaction.response.send_message(content = "Role Creation Successfully created!")
    ...
    async def deny_button(self, interaction:discord.Interaction, button:discord.ui.Button):
        await interaction.response.send_message(content = "Role Creation Successfully denied!")
    ...
    async def show_avatar(self, interaction:discord.Interaction, button:discord.ui.Button):
...
class CustomRole(commands.Cog):
    ...
    async def slash_customrole(self, interaction:discord.Interaction, name: str, color: str, icon: str = None, existing_role: str = None):
        await interaction.response.defer(ephemeral=True)

        level_50_role_name = "Level 50"
        level_50_role = discord.utils.get(interaction.guild.roles, name=level_50_role_name)
        if level_50_role not in interaction.user.roles:
            await interaction.followup.send("You have to be level 50 to use this command.")
            return
    ...
    # Create the custom role
        ...
        role = await guild.create_role(name=name, color=discord.Color(color_int), permissions=permissions)
    ...
    # Set the icon for the role if provided
        ...
        await interaction.followup.send(f"Custom role '{name}' has been sent for review!", ephemeral=True)
        ...
        await admin_channel.send(embed=embed, view=CustomRoleButtons(guild = interaction.guild, role = role, admin_channel=admin_channel, user = interaction.user, existing_role=existing_role, icon = icon))
