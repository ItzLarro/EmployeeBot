...
class Giveaway(commands.Cog):
    ...
    async def slash_giveawaycreate(self, interaction: discord.Interaction, prize: str, duration: str, channel:discord.TextChannel):

        time = convert(duration)
        if time == -1:
            await interaction.response.send_message(f"You didn't use a proper unit when specifying the duration.Please use (s|m|h|d)")
            return
        elif time == -2:
        ...
        await interaction.response.send_message("Giveaway successfully created!", ephemeral=True)
        msg = await channel.send(embed = GiveawayEmbed)
        await msg.add_reaction("<:tc_tada:1102929530794016870>")

        async with aiosqlite.connect(DB) as conn:
            cursor = await conn.cursor()
            await cursor.execute('''INSERT INTO Giveaways (prize, end_time, channel_id, message_id) VALUES (?, ?, ?, ?)''',
    ...
    @app_commands.command(name = "giveawayreroll", description = "Rerolls the winner of a specified giveaway")
    ...
    async def slash_giveawayreroll(self, interaction:discord.Interaction, message_id:str, channel_id:str):
