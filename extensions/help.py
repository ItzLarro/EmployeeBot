...
class HelpTypes(discord.ui.Select):
        def __init__(self):
            ...
            for label, value in self.help_sections.items():
                options.append(discord.SelectOption(label=label, value=label, description=value.get('desc')))

            super().__init__(placeholder='Choose the type of command...', min_values=1, max_values=1, options=options)
...
class Help(commands.Cog):
    ...
    @app_commands.command(name = "help", description = "Lets you know what commands the bot uses, and what they do!")
    async def slash_help(self, interaction: discord.Interaction):
        view = DropdownView()
        embed = discord.Embed(title = "Command Type Selection", description = "Use the dropdown menu to view the type commands you choose.", color = 0x86def2)
