...
class TradePost(commands.Cog):
    ...
    @app_commands.command(name = "posttrade", description = "Post a Trade Advertisement through the bot.")
    ...
    async def posttrade(self, interaction: discord.Interaction, have: str, want: str, tradelink: str):
        ...
        webhook = await interaction.channel.create_webhook(name=interaction.user.name, avatar=avatar_bytes)
        ...
        infoEmbed = discord.Embed(title = "What is this?", description="This is a feature that lets traders post their trade advertisements\n without having to wait the full 6 hour-cooldown.\nTo get access to this yourself check out #patreon.", color=0x313338)
    ...
    @posttrade.error
    async def on_posttrade_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError):
