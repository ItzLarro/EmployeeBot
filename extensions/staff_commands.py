...
class StaffCommands(commands.Cog):
    ...
    @app_commands.command(name = "membercategory", description = "Creates category that counts the server members")
    async def membercategory(self, interaction:discord.Interaction):  
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message("Sorry, only admins can use this command.")
            return
        else:
